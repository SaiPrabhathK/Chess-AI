...
void AI::start()
{
    ...
    // <<-- /Creer-Merge: start -->>
    //gameBoard = GameBoard::parseFen2(game->fen);
}
...
std::string AI::make_move()
{
    ...
    //return gameBoard.getRandomMove();
    
    std::string fenString = game->fen;
    GameBoard c = GameBoard::parseFen2(fenString);
    //cout<<c;
    std::string mymove = c.getRandomMove();
    std::cout << mymove << std::endl;
    std::cout << c.isKingInCheck() << std::endl;
