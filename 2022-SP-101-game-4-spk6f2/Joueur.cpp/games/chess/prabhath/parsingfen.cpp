...
//assigns a value to each move to be used for move ordering.
int Move::moveValue()
{

    return 100 * (!this->isCaptured) 
        + 80 * (this->isPromotion ? this->promotionPieceType : 0) 
        - 60 * (this->isCaptured ? this->capturedPieceType : 0)
        + 90 * this->castlingSide 
        + 30 * this->isEnpassant
        + 20 * this->to 
        - 20 * this->from;
}
...
vector<std::pair<Move, GameBoard>> GameBoard::generateMoves()
{
    ...
    std::sort(validMovesandGameBoards.begin(), validMovesandGameBoards.end(), 
    []( std::pair<Move, GameBoard> & a,  std::pair<Move, GameBoard> & b) -> bool
    { 
        return a.first.moveValue() > b.first.moveValue(); 
    });
...
std::string Algo::timeLimitedIterativeDeepeningABMInMaxWithQuiescenceSearchAndMoveOrdering(std::string orgFenString, double remainingTime)
{
...
int Algo::maxPlayerAB(GameBoard eachMinPlayerGameState, int depth, int alphaValue, int betaValue)
{
    if (isCutOffReached(depth))
    {
        ...
        return quiescentSearch(eachMinPlayerGameState, alphaValue, betaValue);
    }
    int value = INT_MIN;
...
int Algo::minPlayerAB(GameBoard eachMaxPlayerGameState, int depth, int alphaValue, int betaValue)
{
    if (isCutOffReached(depth))
    {
        ...
        return quiescentSearch(eachMaxPlayerGameState, alphaValue, betaValue);
    }
    int value = INT_MAX;
...
//Quiescence Search
int Algo::quiescentSearch(GameBoard gameState, int alphaValue, int betaValue) {
    int standPat = gameState.evaluateState();
    int score;
    if( standPat >= betaValue )
        return betaValue;
    if( alphaValue < standPat )
        alphaValue = standPat;

    std::vector<std::pair<Move, GameBoard>> movesWithTheirGameStates = gameState.generateMoves();

    for(int eachMoveAndGameStatePair = 0; eachMoveAndGameStatePair < movesWithTheirGameStates.size(); eachMoveAndGameStatePair++)
    {
        if(movesWithTheirGameStates[eachMoveAndGameStatePair].first.isCaptured)
        {
        score = -quiescentSearch( movesWithTheirGameStates[eachMoveAndGameStatePair].second.clone(true), -betaValue, -alphaValue );
        }
        else continue;
